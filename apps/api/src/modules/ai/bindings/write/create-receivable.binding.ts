...
@Injectable()
export class CreateReceivableWriteBinding
  implements WriteCapabilityBindingDefinition<CreateManualAccountWriteInput>
{
  readonly capability = 'CREATE_RECEIVABLE' as const;
  readonly version = '1.0.0';
  readonly mode = 'WRITE' as const;
  readonly bindingName = 'create_receivable_canonical@1.0.0';
  ...
  mapInput(
    ...
    if (step.capability !== 'CREATE_RECEIVABLE') {
      throw new BadRequestException('Unexpected capability for CREATE_RECEIVABLE binding');
    }
    ...
    // Only trust server-resolved clientId from enricher/resolver (selectedClientId mapped).
    const clientId =
    ...
    if (!counterpartyName) {
      throw new BadRequestException('CREATE_RECEIVABLE requires counterpartyName');
    }
    if (amount === null || amount <= 0) {
      throw new BadRequestException('CREATE_RECEIVABLE requires a positive amount');
    }

    return {
      counterpartyName,
      concept: asString(args.concept) ?? defaultConcept('RECEIVABLE'),
      amount,
      currency,
      ...
      registerIdempotencyKey: createReceivableIdempotencyKey(context.actionRunId),
    };
  }
  ...
  async execute(
    ...
    if (input.clientId) {
      ...
      if (!client) {
        throw new ConflictException('STALE_CREATE_RECEIVABLE_CLIENT');
      }
    }

    let result;
    try {
      result = await this.manualAccounts.createReceivable(context.tenantId, {
      ...
      if (error instanceof ConflictException) {
        ...
        if (msg.includes('cancelled/deleted')) {
          throw new ConflictException('STALE_CREATE_RECEIVABLE_CANCELLED');
        }
        if (msg.includes('conflicting')) {
          throw new ConflictException('CANONICAL_CREATE_RECEIVABLE_INVARIANT');
        }
        throw error;
      }
      throw error;
    ...
    return {
      ...
      generatedEvents: [
        {
          type: result.replayed ? 'RECEIVABLE_REPLAYED' : 'RECEIVABLE_CREATED',
          at: new Date().toISOString(),
        },
      ],
      receipt: {
        kind: 'ACCOUNT_RECEIVABLE',
        ...
        treasury: 'Sin cambio',
        pnl: 'Sin cambio',
        cxcDelta: `+$${formatMoney(amountStr)}`,
  ...
  private async assertMembership(context: WriteExecutionContext) {
    ...
    if (!membership) {
      throw new ForbiddenException(
        'Authenticated tenant membership required for receivable creation',
      );
    }
  }
}
