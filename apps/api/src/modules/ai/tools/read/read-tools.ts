import { AccountEntryStatus, AccountEntryType, Prisma } from '@prisma/client';
import { z } from 'zod';
...
export function createReadTools(
  ...
  const clientItem = z.object({ id: z.string(), name: z.string(), normalizedName: z.string(), phone: z.string().nullable(), email: z.string().nullable(), openReceivableCount: z.number().int(), openReceivableTotalByCurrency: z.object({ MXN: money, USD: money }), lastActivityAt: z.string().nullable() }).strict();
  const openOutput = z.object({ totalOutstandingByCurrency: z.object({ MXN: money, USD: money }), count: z.number().int(), entries: z.array(accountOutput) }).strict();
  ...
  return [
    ...
    definition({ name: 'get_monthly_profit', description: 'Canonical reconciled monthly profit.', category: 'ANALYTICS', permission: null, canonicalService: 'AnalyticsService.getMonthlyProfit', inputValidator: z.object({ year: z.number().int().min(2000).max(2200), month: z.number().int().min(1).max(12) }).strict(), outputValidator: z.object({ period: z.string(), salesMxn: money, cogsMxn: money, bankCommissionsMxn: money, operatingExpensesMxn: money, netProfitMxn: money, saleCount: z.number().int() }).strict(), execute: async (ctx, input) => { const data = await analytics.getMonthlyProfit(ctx.tenantId, input.year, input.month); return { data, summary: `Net profit ${data.period}: MXN ${data.netProfitMxn}` }; } }),
    definition({ name: 'get_recent_sales', description: 'Recent closed sales including immutable historical snapshots.', category: 'SALES', permission: null, canonicalService: 'HistoryService.getRecentSalesForTools', inputValidator: z.object({ limit, clientId: z.string().optional(), from: z.string().datetime().optional(), to: z.string().datetime().optional() }).strict(), outputValidator: z.object({ items: z.array(z.object({ dealId: z.string(), soldAt: z.string(), agreedPrice: money, computedStatus: z.string(), isHistoricalImport: z.boolean() }).passthrough()) }).strict(), execute: async (ctx, input) => { const items = await history.getRecentSalesForTools(ctx.tenantId, input.limit, input.clientId, input.from, input.to); return { data: { items }, summary: `${items.length} recent sales` }; } }),
  ];
}
