...
import { DEMO_SEED_ACTOR } from './demo-tenant.constants';
...
/**
 * Inserts the canonical MXN demo operational dataset for a tenant that has
 * already been wiped. Does not touch User / TenantUser / Role / Tenant.
 */
export async function seedDemoOperationalData(
  ...
  const staleRule = await prisma.automationRule.create({ data: { tenantId: tenantId, type: AutomationRuleType.STALE_DEAL, isEnabled: true, thresholdDays: staleDealThreshold } });
  const overdueRule = await prisma.automationRule.create({ data: { tenantId: tenantId, type: AutomationRuleType.OVERDUE_PAYMENT, isEnabled: true, thresholdDays: 1 } });
  const agingRule = await prisma.automationRule.create({ data: { tenantId: tenantId, type: AutomationRuleType.AGING_INVENTORY, isEnabled: true, thresholdDays: agingInventoryThreshold } });

  await prisma.automationRun.createMany({
    data: [
      { tenantId: tenantId, ruleId: staleRule.id, status: AutomationRunStatus.SUCCESS, resultCount: staleDealCount, createdAt: daysAgo(1) },
      { tenantId: tenantId, ruleId: overdueRule.id, status: AutomationRunStatus.SUCCESS, resultCount: overduePaymentCount, createdAt: daysAgo(1) },
      { tenantId: tenantId, ruleId: agingRule.id, status: AutomationRunStatus.SUCCESS, resultCount: agingInventoryCount, createdAt: daysAgo(1) },
    ],
  });
